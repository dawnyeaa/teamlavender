...
VaryingsParticle vertParticleUnlit(AttributesParticle input)
{
    ...
    GetParticleTexcoords(texcoord2, output.texcoord2AndBlend, input.texcoords.xyxy, 0.0);
#else
    GetParticleTexcoords(texcoord2, output.texcoord2AndBlend, input.texcoords, input.texcoordBlend);
#endif
#else
    GetParticleTexcoords(texcoord2, input.texcoords.xy);
#endif
    output.texcoord = texcoord2.xyxy;
    output.texcoord.zw -= _DropShadOffset;
...
half4 fragParticleUnlit(VaryingsParticle input) : SV_Target
{
    ...
    half4 topTex = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, input.texcoord.xy);
    half4 bottomTex = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, input.texcoord.zw);
